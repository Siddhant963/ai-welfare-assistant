...
- **No `directUrl`-vs-`url` split.** The installed `@prisma/config` (7.10.0) has no `directUrl` field — `prisma7.config.ts` runs migrations through `DATABASE_URL` alone, verified working against Neon's pooled connection. `DIRECT_DATABASE_URL` is not read anywhere in this codebase; see `docs/production-readiness.md` for the current recommendation on it.
